...
### Opción A — Command Prompt o PowerShell (recomendado en Windows)

```bat
REM Desde la raíz del proyecto
scripts\dev-tours.bat

REM En otra ventana
scripts\dev-frontend.sh   (con Git Bash) o:
cd frontend && npm run dev
```

### Opción B — Git Bash

Los scripts `.sh` detectan Windows automáticamente y delegan a `gradlew.bat`:

```bash
./scripts/dev-tours.sh    # detecta MINGW/MSYS, llama a gradlew.bat internamente
```

### Opción C — Directamente con gradlew.bat
...
### Compatibilidad de versiones Java en Windows

| Java | Gradle 8.10.2 | Estado |
|---|---|---|
| 17 | ✅ | Recomendado LTS |
| 21 | ✅ | Recomendado LTS |
| 23 | ✅ | Soportado |
| 24+ | ⚠️ | Verificar notas de Gradle |

> **Nota**: Si `JAVA_HOME` contiene espacios (ej. `C:\Program Files\Java\...`), usar siempre `gradlew.bat` directamente o los scripts `.bat`. El script POSIX `gradlew` puede fallar con espacios en el path en ciertas versiones de Git Bash.
